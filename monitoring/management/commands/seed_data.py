"""
Django management command to seed sample data.
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        pipeline = self.create_sample_pipeline()

        # Create pipeline segments
        segments = self.create_pipeline_segments(pipeline)

        # Create sample images
        images = self.create_sample_images(pipeline)

        # Create analysis results
        results = self.create_sample_analysis_results(images)

        # Create alerts
        alerts = self.create_sample_alerts(results)

        # Create monitoring configuration
        config = self.create_monitoring_configuration(pipeline)
    ...
    def create_pipeline_segments(self, pipeline):
        ...
        self.stdout.write(f"Created {len(segments)} pipeline segments")
        return segments

    def create_sample_images(self, pipeline):
        ...
        self.stdout.write(f"Created {len(images)} satellite images")
        return images
    ...
    def create_sample_analysis_results(self, images):
        ...
        self.stdout.write(f"Created {len(results)} analysis results")
        return results

    def create_sample_alerts(self, results):
        ...
        self.stdout.write(f"Created {len(alerts)} monitoring alerts")
        return alerts

    def create_monitoring_configuration(self, pipeline):
