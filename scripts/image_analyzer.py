...
class VandalismDetector:
    ...
    def detect_vandalism(
        self, current_image: np.ndarray, reference_image: np.ndarray = None
    ) -> List[AnalysisResult]:
...
class ImageAnalyzer:
    ...
    def analyze_image(
        ...
        if "vandalism" in analysis_types:
            vandalism_results = self.vandalism_detector.detect_vandalism(
    ...
    def batch_analyze(
        self, image_paths: List[str], analysis_types: List[str] = None
...
def analyze_satellite_image_task(image_id: str, analysis_types: List[str] = None):
    ...
    try:
        ...
        for result in results:
            ...
            pixel_lat = (
                bounds.extent[1]
                + (result.location[0] / satellite_image.image_file.height) * lat_range
            )
            pixel_lon = (
                bounds.extent[0]
                + (result.location[1] / satellite_image.image_file.width) * lon_range
            )
            ...
            logger.info(
                f"Created analysis result {analysis_result.id} for {result.analysis_type}"
            )
...
if __name__ == "__main__":
    ...
    results = analyzer.analyze_image(
        "path/to/image.jpg", analysis_types=["leak_detection", "oil_spill"]
    )

    print(f"Found {len(results)} anomalies:")
    for result in results:
        print(
            f"- {result.analysis_type}: {result.confidence_score:.2f} confidence ({result.severity})"
        )
