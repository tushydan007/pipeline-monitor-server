...
class AnalysisResultFilter(django_filters.FilterSet):
    ...
    analysis_type = django_filters.ChoiceFilter(
        choices=AnalysisResult.ANALYSIS_TYPE_CHOICES
    )
    ...
    confidence_min = django_filters.NumberFilter(
        field_name="confidence_score", lookup_expr="gte"
    )
    confidence_max = django_filters.NumberFilter(
        field_name="confidence_score", lookup_expr="lte"
    )
...
class MonitoringAlertFilter(django_filters.FilterSet):
    ...
    date_to = django_filters.DateTimeFilter(field_name="created_at", lookup_expr="lte")
    pipeline = django_filters.UUIDFilter(
        field_name="analysis_result__satellite_image__pipeline"
    )
