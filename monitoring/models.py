...
class Pipeline(models.Model):
    ...
    route = gis_models.LineStringField(null=True, blank=True, help_text="Pipeline route geometry")
    default_bounds = gis_models.PolygonField(null=True, blank=True, help_text="Automatically calculated satellite image bounds from route")
    ...
    def calculate_default_bounds(self, buffer_km=1.0):
        ...
        try:
            ...
            extent = self.route.extent  # (minx, miny, maxx, maxy) => (west, south, east, north)
...
class SatelliteImage(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    pipeline = models.ForeignKey(Pipeline, on_delete=models.CASCADE, related_name='satellite_images')
    # User associations
    # The end user whose dashboard should display results
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='user_satellite_images', null=True, blank=True)
    # The admin/staff who uploaded the image
    uploaded_by = models.ForeignKey(User, on_delete=models.SET_NULL, related_name='uploaded_satellite_images', null=True, blank=True)
    ...
    # Geographic bounds
    bounds = gis_models.PolygonField(null=True, blank=True, help_text="Image coverage area")
...
class AnalysisResult(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    satellite_image = models.ForeignKey(SatelliteImage, on_delete=models.CASCADE, related_name='analysis_results')
    ...
    # Geographic information
    detected_location = gis_models.PointField(null=True, blank=True, help_text="Location of detected anomaly")
    affected_area = gis_models.PolygonField(blank=True, null=True, help_text="Area affected by the anomaly")
...
class PipelineSegment(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    pipeline = models.ForeignKey(Pipeline, on_delete=models.CASCADE, related_name='segments')
...
class MonitoringConfiguration(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    pipeline = models.OneToOneField(Pipeline, on_delete=models.CASCADE, related_name='monitoring_config')
...
class SystemSettings(models.Model):
    ...
    site_description = models.TextField(default='Advanced pipeline monitoring and analysis platform')
