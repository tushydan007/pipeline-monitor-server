...
class TimeSeriesAnomalyDetector:
    ...
    def _detect_statistical_anomalies(
        ...
        try:
            ...
            for idx in iqr_anomalies:
                distance = min(
                    abs(values[idx] - lower_bound), abs(values[idx] - upper_bound)
                )
    ...
    def _merge_anomalies(
        ...
        for next_anomaly in anomalies[1:]:
            ...
            time_diff = abs(
                (next_anomaly.timestamp - current_anomaly.timestamp).total_seconds()
            )

            if (
                time_diff < 3600
                and next_anomaly.anomaly_type == current_anomaly.anomaly_type
            ):
                # Merge anomalies
                current_anomaly.confidence_score = max(
                    current_anomaly.confidence_score, next_anomaly.confidence_score
                )
                current_anomaly.description += (
                    f" (merged with {next_anomaly.timestamp})"
                )
            else:
...
class SpatialAnomalyDetector:
    ...
    def _detect_cluster_anomalies(
        self, coords: np.ndarray, values: np.ndarray, cluster_labels: np.ndarray
    ) -> List[AnomalyDetectionResult]:
...
class EnsembleAnomalyDetector:
    ...
    def _detect_ml_anomalies(
        self, data: pd.DataFrame, lat_column: str, lon_column: str, value_column: str
    ) -> List[AnomalyDetectionResult]:
            ...
            for idx, (is_anom, score) in enumerate(zip(is_anomaly, anomaly_scores)):
                if is_anom:
                    ...
                    anomaly = AnomalyDetectionResult(
                        ...
                        severity=severity,
                        location=(
                            data.iloc[idx][lat_column],
                            data.iloc[idx][lon_column],
                        ),
                        timestamp=datetime.now(),  # Placeholder
    ...
    def _merge_anomaly_group(
        self, group: List[AnomalyDetectionResult]
        ...
        base.severity = max(
            group, key=lambda x: severity_levels.get(x.severity, 0)
        ).severity
...
class AnomalyDetectionManager:
    ...
    def detect_pipeline_anomalies(
        self, pipeline_id: str, start_date: datetime, end_date: datetime
    ) -> List[AnomalyDetectionResult]:
        """
    ...
    def get_anomaly_statistics(self) -> Dict[str, Any]:
        ...
        total_detections = sum(
            entry["anomaly_count"] for entry in self.detection_history
        )
        ...
        return {
            ...
            "last_detection": max(
                entry["timestamp"] for entry in self.detection_history
            ),
        }
        ...
        for anomaly in anomalies:
            if anomaly.confidence_score > 0.7:
                ...
                analysis_result = AnalysisResult.objects.filter(
                    satellite_image__pipeline_id=pipeline_id,
                    detected_location__distance_lte=Point(
                        anomaly.location[1], anomaly.location[0]
                    ),
                ...
                if analysis_result:
                    ...
                    logger.info(
                        f"Created alert {alert.id} for anomaly {anomaly.anomaly_type}"
                    )

        logger.info(
            f"Completed anomaly detection for pipeline {pipeline_id}: {len(anomalies)} anomalies"
        )
...
if __name__ == "__main__":
    ...
    for anomaly in anomalies:
        print(
            f"- {anomaly.anomaly_type}: {anomaly.confidence_score:.2f} confidence ({anomaly.severity})"
        )
