...
class PipelineAdmin(GISModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('id', 'name', 'description', 'length_km', 'diameter_mm', 'material', 'status')
        }),
...
class SatelliteImageAdmin(GISModelAdmin):
    list_display = ['satellite_name', 'image_date', 'pipeline', 'processing_status', 'resolution_m']
    ...
    fieldsets = (
        ('Image Information', {
            'fields': ('id', 'pipeline', 'image_date', 'satellite_name', 'sensor', 'resolution_m')
        }),
        ('Files', {
            'fields': ('image_file', 'image_preview', 'thumbnail')
        }),
        ('Geographic Information', {
            'fields': ('bounds', 'center_point')
        }),
        ('Processing', {
            'fields': ('processing_status', 'source_api', 'api_image_id')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class AnalysisResultAdmin(GISModelAdmin):
    ...
    fieldsets = (
        ('Analysis Information', {
            'fields': ('id', 'satellite_image', 'analysis_type', 'confidence_score', 'severity')
        }),
        ('Location', {
            'fields': ('detected_location', 'affected_area')
        }),
        ('Details', {
...
class MonitoringAlertAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Status', {
            'fields': ('is_resolved', 'sent_at', 'recipients')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class PipelineSegmentAdmin(GISModelAdmin):
    list_display = ['segment_name', 'pipeline', 'segment_number', 'risk_level', 'terrain_type']
    ...
    fieldsets = (
        ('Segment Information', {
            'fields': ('id', 'pipeline', 'segment_name', 'segment_number', 'length_km')
        }),
        ('Geographic Information', {
            'fields': ('start_point', 'end_point', 'geometry')
        }),
        ('Monitoring', {
            'fields': ('monitoring_frequency_hours', 'last_monitored')
        }),
        ('Risk Assessment', {
            'fields': ('risk_level', 'terrain_type')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class MonitoringConfigurationAdmin(admin.ModelAdmin):
    list_display = ['pipeline', 'nasa_api_enabled', 'analysis_enabled', 'alerts_enabled']
    ...
    fieldsets = (
        ...
        ('API Settings', {
            'fields': ('nasa_api_key', 'nasa_api_enabled')
        }),
        ('Analysis Settings', {
...
class ActivityLogAdmin(admin.ModelAdmin):
    ...
    search_fields = ['action', 'resource_name', 'user__email', 'user__first_name', 'user__last_name']
    readonly_fields = ['id', 'created_at']
    date_hierarchy = 'created_at'
    
    fieldsets = (
        ('Activity Information', {
            'fields': ('id', 'user', 'action', 'success')
        }),
...
class SystemSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('General Settings', {
            'fields': ('site_name', 'site_description', 'timezone', 'language', 'theme')
        }),
        ('Notification Settings', {
            'fields': ('email_notifications', 'sms_notifications', 'push_notifications', 'alert_email', 'alert_phone')
        }),
        ('Analysis Settings', {
            'fields': ('analysis_confidence_threshold', 'auto_analysis_enabled', 'analysis_retention_days')
        }),
        ('Security Settings', {
            'fields': ('session_timeout', 'password_min_length', 'require_2fa', 'failed_login_lockout')
        }),
        ('API Settings', {
            'fields': ('api_rate_limit', 'api_key_expiry_days')
        }),
        ('Storage Settings', {
            'fields': ('max_file_size_mb', 'storage_quota_gb', 'auto_cleanup_enabled')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
