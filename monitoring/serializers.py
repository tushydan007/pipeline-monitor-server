...
class PipelineSerializer(serializers.ModelSerializer):
    ...
    def get_route_coordinates(self, obj):
        """Return route coordinates as array of [lat, lon] pairs for frontend display"""
        if not obj.route:
            # Fallback to start and end points if no route
            if obj.start_point and obj.end_point:
                logger.info(f"Pipeline {obj.id}: No route, using start/end points")
                return [[obj.start_point.y, obj.start_point.x], [obj.end_point.y, obj.end_point.x]]
            return None

        coords_list = None
        tolerance = 1e-10  # Tolerance for duplicate detection

        # Method 1: Use GeoJSON property (most reliable in Django GEOS)
        try:
            if hasattr(obj.route, "geojson"):
                geojson_data = obj.route.geojson
                # geojson property returns dict in newer Django versions, string in older
                if isinstance(geojson_data, str):
                    geojson_data = json.loads(geojson_data)
                
                if isinstance(geojson_data, dict) and geojson_data.get("type") == "LineString":
                    coordinates = geojson_data.get("coordinates", [])
                    if coordinates and len(coordinates) > 0:
                        ...
                        coords_list = []
                        for coord in coordinates:
                            if len(coord) >= 2 and coord[0] is not None and coord[1] is not None:
                                lat = float(coord[1])
                                lon = float(coord[0])
                                # Only add if different from previous coordinate (avoid duplicates)
                                if len(coords_list) == 0:
                                    coords_list.append([lat, lon])
                                else:
                                    last_coord = coords_list[-1]
                                    if not (
                                        abs(last_coord[0] - lat) < tolerance and
                                        abs(last_coord[1] - lon) < tolerance
                                    ):
                                        coords_list.append([lat, lon])
                        
                        if len(coords_list) >= 2:
                            logger.info(f"Pipeline {obj.id}: Extracted {len(coords_list)} coordinates using geojson property")
                            return coords_list
        except (AttributeError, TypeError, ValueError, json.JSONDecodeError) as e:
            logger.warning(f"Pipeline {obj.id}: Error with geojson property: {str(e)}")

        # Method 2: Try json property (alternative)
        try:
            if hasattr(obj.route, "json") and coords_list is None:
                geojson_str = obj.route.json
                if geojson_str:
                        ...
                        if coordinates and len(coordinates) > 0:
                            ...
                            for coord in coordinates:
                                if len(coord) >= 2 and coord[0] is not None and coord[1] is not None:
                                    lat = float(coord[1])
                                    lon = float(coord[0])
                                    # Only add if different from previous coordinate
                                    if len(coords_list) == 0:
                                        coords_list.append([lat, lon])
                                    else:
                                        last_coord = coords_list[-1]
                                        if not (
                                            ...
                                            coords_list.append([lat, lon])
                            if len(coords_list) >= 2:
                                logger.info(f"Pipeline {obj.id}: Extracted {len(coords_list)} coordinates using json property")
                                return coords_list
        except (AttributeError, TypeError, ValueError, json.JSONDecodeError) as e:
            logger.warning(f"Pipeline {obj.id}: Error with json property: {str(e)}")

        # Method 3: Use coords property directly
        try:
            if hasattr(obj.route, "coords") and coords_list is None:
                # Convert to list immediately to capture all coordinates
                coords_tuples = list(obj.route.coords)
                coords_list = []
                ...
                if len(coords_list) >= 2:
                    logger.info(f"Pipeline {obj.id}: Extracted {len(coords_list)} coordinates using coords property")
                    return coords_list
        except (AttributeError, TypeError, ValueError) as e:
            logger.warning(f"Pipeline {obj.id}: Error using coords property: {str(e)}")

        # Fallback to start and end points if extraction failed
        if obj.start_point and obj.end_point:
            logger.warning(f"Pipeline {obj.id}: All extraction methods failed, falling back to start/end points")
            return [
                [obj.start_point.y, obj.start_point.x],
                [obj.end_point.y, obj.end_point.x],
            ]
        
        logger.error(f"Pipeline {obj.id}: Could not extract any coordinates")
        return None
